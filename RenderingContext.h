...
public:
    ...
    static const int RADIUS_FILL = 0;
    static const int DIFFUSE_FILL = 1;


    static const int NO_OF_SHADERS = 2;
    int currentShader;
    int glprogram[NO_OF_SHADERS];
    int diffusehandle[NO_OF_SHADERS], poshandle[NO_OF_SHADERS], colhandle, model;

    int mvhandle, mvphandle[NO_OF_SHADERS];
...
inline RenderingContext::RenderingContext() : mvhandle(-1), depth(0)
{
...
inline void RenderingContext::changeShader(int index)
{
    assert(index >= 0 && index < NO_OF_SHADERS);
    glUseProgram(glprogram[index]);
    currentShader = index;
}